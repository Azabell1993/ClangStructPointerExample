...
#define DB_USER "root"
#define DB_PASS "패스워드를 입력하세요."
#define DB_NAME "아이디를 입력하세요."
...
int main()
{
    ...
    int getFpNum;
    getFpNum = nameOuterObj.select_name_number_;
    int hakbun;
    int age;
    int inputdate;
    int outputdate;
    ...
    query_stat = mysql_query(connection, "select * from school");
    if (query_stat != 0)
    {
    ...
    {
        printf("%s %s %s %s %s", sql_row[0], sql_row[1], sql_row[2], sql_row[3], sql_row[4]);
        printf("\n");
    }
    printf("***************************************************************\n");
    mysql_free_result(sql_result);

    printf(" 호출할 이름과 학교 함수포인터 인수 값 : ");
    scanf("%d", &getFpNum);

    // 부모 유사 클래스 MEMBER에 상속된 자식 클래스 new 생성자 호출
    MEMBER_NAME* nameObj = new_name(getFpNum);
    ...
    //scanf("%d %d %d %d", &hakbun, &age, &inputdate, &outputdate);
    printf(" 학 번 : "); scanf("%d", &hakbun);
    printf(" 나 이 : "); scanf("%d", &age);
    printf(" 입학날짜 : "); scanf("%d", &inputdate);
    printf(" 졸업날짜 : "); scanf("%d", &outputdate);
    MEMBER* memOuterObj = new_member(hakbun, nameObj, schoolObj, age, inputdate, outputdate);

    sprintf(query, "insert into school values ""('%d', '%d', '%d', '%d','%d')",
        getFpNum, hakbun, age, inputdate, outputdate);

    // 자식 클래스의 생성자 호출, 이름과 학교 이름이 동시에 출력이 되도록 생성자를 전방에서 선언해둠.
    NAME_INIT(nameObj);
    NAME_SET_(nameObj, getFpNum);
    NAME_GET_(nameObj);
